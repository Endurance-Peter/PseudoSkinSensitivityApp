    ...
    public class WellboreRadiusRegressionViewModel : BindableBase
    {
        ...
        private void RegressAction(string parameter)
        {
            if (parameter == ParameterVariables.WELLBORE_RADIUS)
            {
                ...
                var sensitivity = fetchSeudoskin.Result.SensitivityResults.Where(x => x.SensititvityVariable == PseudoSkinDomain.Models.SensititvityVariable.WellboreRadius).SelectMany(x => x.Results);
